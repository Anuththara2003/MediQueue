...
$(function() {
    ...
    function handleNext() {
        if (currentStep === 1) {
            ...
            .done(function(patientData) {
                Toast.fire({ icon: 'success', title: 'Registered patient found!' });
                $('#tokenForm').data('patient-id', patientData.id);
                $('#patientName').val(`${patientData.firstName} ${patientData.lastName}`).prop('readonly', true);
                $('#dob').val(patientData.dateOfBirth).prop('readonly', true);
                $('#gender').val(patientData.gender).prop('readonly', true);
            ...
            .fail(function(jqXHR) {
                if (jqXHR.status === 404) {
                    Toast.fire({ icon: 'info', title: 'New patient. Please enter details.' });
                    $('#patientName').val('').prop('readonly', false);
                    $('#dob').val('').prop('readonly', false);
                    $('#gender').val('').prop('readonly', false);
                    // Generate a temporary negative ID for new patients
                    $('#tokenForm').data('patient-id', -1); 
                    currentStep++;
                } else {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'An error occurred while checking the number. Please try again.',
                    });
                }
            })
            .always(function() {
    ...
    function handleSubmit(event) {
        ...
        const patientId = $('#tokenForm').data('patient-id');
        const doctorId = $('#doctor').val();
        const appointmentDate = $('#appointmentDate').val();
        
        // Frontend Validation
        if (!patientId) {
            Swal.fire('Error', 'Patient could not be identified. Please start over.', 'error').then(resetForm);
            return;
        }
        if (!doctorId || !appointmentDate) {
            Swal.fire('Incomplete Information', 'Please ensure you have selected a doctor and an appointment date.', 'warning');
            return;
        }
        ...
        $.ajax({
            url: 'http://localhost:8080/api/v1/patient/tokens',
            type: 'POST',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('jwtToken')}`,
                'Content-Type': 'application/json'
        ...
        .done(function(response) {
            Swal.fire({
                ...
                html: `Your token has been successfully registered.<br>Your token number is: <strong>${response.tokenNumber}</strong>`,
            }).then(() => {
                closeModal();
            });
        })
    ...
    function loadDoctorsIntoDropdown() {
        ...
        $doctorSelect.prop('disabled', true).html('<option value="">Loading doctors...</option>');
        $.ajax({
            url: 'http://localhost:8080/api/v1/patient/details/doctors',
            type: 'GET',
            headers: { 'Authorization': `Bearer ${JWT_TOKEN}` }
        })
        .done(function(doctors) {
    ...
    function updateButtons() {
        ...
        $nextBtn.toggle(currentStep < $formSteps.length);
        $submitBtn.toggle(currentStep === $formSteps.length);
        $nextBtn.text(currentStep === 1 ? 'Check' : 'Next');
    }
